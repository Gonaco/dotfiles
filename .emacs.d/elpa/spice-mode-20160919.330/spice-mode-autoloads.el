...

;;;### (autoloads nil "spice-mode" "spice-mode.el" (22687 17854 0
;;;;;;  0))
...
(custom-declare-variable 'spice-hide-line-prefix '(concat (regexp-quote (concat comment-start (if (boundp 'comment-padding) (if (integerp comment-padding) (make-string comment-padding 32) comment-padding) " "))) "[a-z\\*!$0-9+\\.]") "*Regexp string describing lines that are commented out and will be\nhidden. The regexp is matched to the beginning of a line, the ^ is\nadded automatically. The initialization of this variable is handled\nin `spice-hide-init', which is after the setting of `comment-start'\nand `comment-padding' variables." :initialize (lambda (variable value) (message "Deferring initialization of %s with %s to spice-hide-init" variable value)) :group 'spice-hide :type 'string)
